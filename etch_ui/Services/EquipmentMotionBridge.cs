...
public sealed class EquipmentMotionBridge
{
    ...
    public void SyncTransferMotion(TmTransferSimulator transfer, string equipmentState)
    {
        ...
        _motion.ApplyTransferDiagnostics(
            ...
            FormatBladeSummary("Vacuum TM", transfer.VacuumActiveBladeSlot, transfer.VacuumCarryingSlotA, transfer.VacuumCarryingSlotB, transfer.VacuumBladeCapacity, transfer.VacuumIsRotatingBlade),
            FormatBladeSummary("EFEM", transfer.EfemActiveBladeSlot, transfer.EfemCarryingSlotA, transfer.EfemCarryingSlotB, transfer.EfemBladeCapacity, false),
    ...
    {
        string phase = transfer.PhaseHint;
        ThroughputKpiSnapshot kpi = transfer.KpiSnapshot;
        return $"LOT {transfer.LotCompletedCount}/{transfer.LotTargetCount} · {phase} · {kpi}";
    }
    ...
    private static string FormatHoldSummary(TmTransferSimulator transfer)
    {
        ...
        {
            return "Hold · Side Stg 만석 · 카세트 교체 필요";
        }

        if (vacPending > 0)
        {
            return $"Hold · Vacuum drop 대기 {vacPending}건 · PM/BM 수용 조건 확인";
        }

        if (vacQueue > 0 || efemQueue > 0)
        {
            return $"Queue · EFEM {efemQueue} / Vacuum {vacQueue} · Blade {vacBlades}/{transfer.VacuumBladeCapacity}";
        }

        return $"Hold 없음 · Blade {vacBlades}/{transfer.VacuumBladeCapacity}";
    }
}
